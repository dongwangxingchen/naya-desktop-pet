...
这份计划按依赖关系和可验证的纵向功能切分。用户已经确认不购买付费绑定工具，也不聘请专业绑定师。Godot 作为桌宠主程序已经确定；角色作者工具和实时运行方式必须先通过临时小样，不能在正式角色上边试边补。

## 架构决定

- 保留 Godot 4.6.3 作为角色、动画、窗口、气泡和管理 UI 的主程序。
- 角色渲染通过 `CharacterRenderer` 接口隔离，允许固定分层、同源烘焙动作和未来参数化运行时互换。
- 作者工具优先验证 `nijigenerate MCP`，Blender/Godot NativeRig 是安全回退；测试版不能直接接触唯一一份正式素材。
- 第一版实时面部保留固定分层基线；骨骼与网格只用于经过验证的辫子、羽毛和披肩等柔性部位。
...
- 明确必须保存 Krita `.kra`、PSD/PSB 或等价分层源文件、绑定工程、导出清单与 Godot 场景。
...
说明：只准备脸底、双眼、上下眼皮、瞳孔和前发，不使用正式纳雅母版，也不补做全身。

验收条件：

- 所有图层画布、原点和缩放一致。
- 测试件可以明确测出闭眼穿帮、网格翻折和锚点位移。
- 资源检查器能发现故意制造的尺寸与透明边缘错误。

验证：输出测试件叠合图、图层清单和哈希。

依赖：0.2。

计划文件：`tests/fixtures/rig-spike/layers/`、`tests/fixtures/rig-spike/manifest.json`、`tools/validate_character_assets.py`。

预计范围：S。

### 任务 1.2：`nijigenerate MCP` 作者工具小样

说明：在隔离目录中让 Codex 通过 MCP 导入固定尺寸 PNG 图层文件夹，并完成节点整理、网格、`EyeOpen`、`EyeXY`、绑定、截图、保存和透明导出。

验收条件：

- 不依赖屏幕坐标点击即可完成核心操作。
- 开眼、半闭、闭眼和五个视线方向都能自动截图，支持网格覆盖检查。
- 保存、退出、重开后参数和绑定读回一致。
- 连续三轮操作不崩溃；所有阶段另存版本，原始测试件不被覆盖。

验证：保存 MCP 调用日志、`.inx`、`.inp`、透明 PNG、差分报告和失败复现步骤。

依赖：用户同意安装并隔离测试 `nijigenerate v1.0.0-beta2`；1.1。

计划文件：`tools/rig_spike/`、`artifacts/rig-spike/nijigenerate/`、`docs/character/nijigenerate-spike-report.md`。

预计范围：M。

### 任务 1.3：Godot 运行基线小样

说明：用同一测试件验证两种低风险运行方式：固定分层实时眨眼/视线，以及同一绑定工程导出的透明动作序列。

验收条件：

- 眨眼期间脸底与非眼部像素不变化。
- 瞳孔实时跟随，头部跟随可延迟和限幅。
- 烘焙序列首尾锚点一致、透明边缘干净，并能按需加载和释放。
- 两种方式都能通过 30 FPS、低功耗和长时间稳定性测试。

验证：差分报告、60 秒预览、CPU/内存采样和保存重开测试。

依赖：1.1；如果 1.2 成功，则同时验证其导出结果。

计划文件：`src/character/character_renderer.gd`、`src/character/layered/face_rig.tscn`、`src/character/baked/baked_action_player.gd`、`tests/character/runtime_baseline_probe.gd`。

预计范围：M。

### 任务 1.4：实时参数运行时可行性门

说明：只评估 `nicxlive -> Godot GDExtension` 的最小闭环，不在这一任务实现完整生产运行时。

验收条件：

- 能加载最小 `.inp/.inx`，透明渲染并实时改变一个参数。
- 明确纹理、顶点、遮罩、混合模式、动态合成和跨平台打包的缺口。
- 给出可估算的实现清单；不能把未实现能力藏在后续阶段。

验证：最小演示、崩溃日志、性能采样和回退说明。

依赖：1.2 通过；如果 1.2 失败则跳过。

计划文件：`spikes/nicxlive-godot/`、`docs/character/nicxlive-runtime-spike.md`。

预计范围：M。

### 任务 1.5：Blender/Godot NativeRig 回退小样

说明：如果 `nijigenerate` 作者工具或实时路径未过门，使用 Blender Python 自动建立最小网格、骨架、Shape Keys 和 Godot 导出；不把早期第三方插件直接混入主工程。

验收条件：

- 脚本可从干净目录重复生成同一结果。
- 眨眼、瞳孔、轻呼吸和一个柔性部位可独立叠加。
- 导出后不依赖 Blender 或第三方插件即可在 Godot 运行。

验证：无界面重建、导出资源差分和 Godot 导入测试。

依赖：1.3；在需要回退时执行。

计划文件：`tools/blender_rig/`、`spikes/blender-native-rig/`、`docs/character/blender-native-rig-spike.md`。

预计范围：M。

### 任务 1.6：锁定工具链并制作正式主姿势

说明：按小样证据选择作者工具和运行方式后，才从同一张获批纳雅母版建立正式分层、补画隐藏区域并制作第一套参数。

验收条件：

- 用户确认角色身份、肤色、细节和慵懒主姿势。
- 眨眼、视线、眉毛、嘴型、轻呼吸和柔性部位可以同时叠加。
- 所有源文件、导出清单、绑定参数和版本可继续编辑。
- 一套测试服装切换 100 次，无闪烁、残层或动作中断。

验证：版本化验收报告、60 秒预览、面部差分、性能预算和 8 小时稳定性测试。

依赖：1.3，以及适用时的 1.4 或 1.5；用户批准最终工具链与主姿势。

计划文件：`assets/character/master/`、`src/character/`、`docs/character/rig-pipeline-acceptance.md`、`tools/report_rig_metrics.py`。
...
| `nijigenerate` 测试版崩溃或损坏工程 | 绑定工作丢失 | 只操作副本、阶段另存、保存重开验证，并保留 Blender/Godot 回退 |
| nijilive 没有成熟 Godot 插件 | 实时参数模型无法直接进入桌宠 | 第一版保留固定分层与同源烘焙基线，实时桥接单独过门 |
| 自主分层与绑定精度不足 | 动作僵硬或出现空洞 | 单一主姿势、小步验收、补画隐藏区域和自动差分 |
...
- 已确认：不购买付费绑定工具，不聘请专业绑定师；Godot 继续作为桌宠主程序。
- 尚未确认：是否允许安装并隔离测试免费的 `nijigenerate v1.0.0-beta2`。在得到同意前，不安装测试版，也不让它接触正式角色素材。
